...
Usage
-----
    uv run python scripts/gen_linkml.py [TTL_FILE ...] [-d OUTPUT_DIR]

100% coverage goals:
  - Every owl:Class                              -> LinkML class
  - Every owl:ObjectProperty / DatatypeProperty  -> LinkML slot
  - Every owl:AnnotationProperty (gistl: ns)      -> LinkML slot
...
LMODEL_NS = "https://w3id.org/model/gistl/"
LMODEL_BASE = "https://w3id.org/model/gistl"
...
def extract_classes(g: Graph) -> dict[str, dict]:
    """Extract owl:Class entries defined in the gistl: namespace."""
    ...
    for cls in g.subjects(RDF.type, OWL.Class):
        ...
        # --- is_a: first simple named gistl: superclass ---
        simple_parents = [
        ...
        see_also_nodes = list(g.objects(cls, RDFS.seeAlso))
        if see_also_nodes:
            see_also_vals = []
            for n in see_also_nodes:
                if isinstance(n, URIRef):
                    curie = uri_to_curie(n)
                    see_also_vals.append(curie if curie else str(n))
                elif isinstance(n, Literal):
                    see_also_vals.append(str(n))
            if see_also_vals:
                entry["see_also"] = see_also_vals
...
def extract_slots(g: Graph) -> dict[str, dict]:
    """Extract all property definitions in the gistl: namespace."""
    slots: dict[str, dict] = {}
    ...
    for prop_type, kind in prop_types.items():
        for prop in g.subjects(RDF.type, prop_type):
            ...
            if kind == "object":
                entry["implements"] = ["owl:ObjectProperty"]
            elif kind == "annotation":
                entry["implements"] = ["owl:AnnotationProperty"]
            else:
                entry["implements"] = ["owl:DatatypeProperty"]
            ...
            if sub_of:
                ...
                if len(sub_of) > 1:
                    entry["notes"] = entry.get("notes", []) + [
                        "OWL additional subPropertyOf (gistl:): "
            ...
            if kind == "datatype":
                ...
                # gistl:rangeIncludes (soft range hints) — use when no hard rdfs:range
            ...
            if OWL.AsymmetricProperty in all_types:
                entry["asymmetric"] = True
            if OWL.ReflexiveProperty in all_types:
                entry["reflexive"] = True
            if OWL.IrreflexiveProperty in all_types:
                entry["irreflexive"] = True
            if OWL.FunctionalProperty in all_types:
                # FunctionalProperty: each subject has at most one value.
                # LinkML has no direct metamodel slot; capture via `maximum_cardinality: 1`
                # and an annotation. gen-owl 1.9 does not re-emit owl:FunctionalProperty
                # from these, but the semantics are preserved in the schema.
                entry["maximum_cardinality"] = 1
                entry.setdefault("annotations", {})["owl_functional"] = True
                entry["notes"] = entry.get("notes", []) + [
            ...
            if OWL.InverseFunctionalProperty in all_types:
                ...
                entry.setdefault("annotations", {})["owl_inverse_functional"] = True
            ...
            # --- see_also (rdfs:seeAlso) ---
            see_also_nodes = list(g.objects(prop, RDFS.seeAlso))
            if see_also_nodes:
                see_also_vals = []
                for n in see_also_nodes:
                    if isinstance(n, URIRef):
...
def extract_enums(g: Graph) -> dict[str, dict]:
    ...
    Groups them by their rdf:type (gistl: class) into separate enum entries.
    """
    enums: dict[str, dict] = {}
    ...
    for gist_type, individuals in sorted(groups.items()):
        ...
        for ind in sorted(individuals, key=str):
            ...
            # --- notes from editorialNote ---
            ind_ed_notes = get_literals(g, ind, SKOS.editorialNote)
            if ind_ed_notes:
                pv_entry["notes"] = ind_ed_notes

            pv[val_key] = pv_entry

        enums[enum_name] = {
            "description": f"Named instances of gistl:{gist_type} from gist reference data.",
...
def extract_rdfs_annotations(
    ...
    """
    Extract rdfs:label and rdfs:comment for gistl: entities.
    ...
    # Collect all gistl: subjects that have rdfs:label or rdfs:comment
...
def extract_sub_class_assertions(g: Graph) -> dict[str, dict]:
    """
    Extract rdfs:subClassOf triples for gistl: classes into class stubs with is_a.
...
_ELEMENT_KEY_ORDER = [
    ...
    "inverse", "transitive", "symmetric", "asymmetric", "antisymmetric",
    "reflexive", "irreflexive", "disjoint_with",
    # deprecation
...
def build_gist_schema(version: str = "14.1.0") -> dict:
    """Build the top-level gistl.yaml that imports all compatible module schemas."""
    schema: dict[str, Any] = {
        "id": LMODEL_BASE,
        "name": "gistl",
        "title": "gistl",
...
def generate_per_file_schemas(
    ...
    # ---- 6. gistl.yaml (main entry-point, imports core + media_types + prefix_declarations) ----
    print("Building gistl.yaml ...", file=sys.stderr)
    gist_schema = build_gist_schema(version=version)
    _write_schema_file(gist_schema, output_dir / "gistl.yaml")
...
def main() -> None:
    ...
    default_output_dir = Path(__file__).parent.parent / "src/gistl/schema"
